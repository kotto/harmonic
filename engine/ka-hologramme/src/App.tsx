  ...
  return (
    <div className="phone">
      ...
      <div className="screen">
        ...
        {screen === 'profile' && (
          <ProfileScreen
            ...
            kaVoice={kaVoice}
            onToggleVoice={() => setKaVoice(!kaVoice)}
          />
        )}
      </div>
      ...
      <div className={`ka-indicator ${kaVoice ? '' : 'mute'}`}>
        ...
        KA présent{kaVoice ? ' · Mode vocal' : ' · Silencieux'}
      </div>

      {/* Nav */}
      <nav className="nav">
        {(['home', 'chat', 'profile'] as const).map((s) => (
          <button key={s} className={`nav__item ${screen === s ? 'on' : ''}`} onClick={() => setScreen(s)}>
            {s === 'home' && <svg viewBox="0 0 24 24"><path d="M3 13h1v7c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-7h1a1 1 0 0 0 .7-1.7l-9-8.5a1 1 0 0 0-1.4 0l-9 8.5A1 1 0 0 0 3 13z"/></svg>}
            {s === 'chat' && <svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/></svg>}
            {s === 'profile' && <svg viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>}
