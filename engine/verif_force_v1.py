# -*- coding: utf-8 -*-
"""
FORCE V1 — exécution machine du dépôt DEPOT_FORCE_V1.md (28/08/2026)
...
Sortie : resultat_force_v1.json (toutes les lectures, y compris les quasi-échecs).
"""
...
DEPOT = os.path.join(RACINE, "DEPOT_FORCE_V1.md")
SORTIE = os.path.join(RACINE, "resultat_force_v1.json")
...
def bessel_table_recurrence(n_max, a):
    """Route indépendante C2 — récurrence descendante Jₙ₋₁ = (2n/a)Jₙ − Jₙ₊₁,
    ensemencée en haut, normalisée par J₀ (série)."""
    J = {n_max + 1: 0.0, n_max: 1.0e-300}
    for n in range(n_max, 0, -1):
        J[n - 1] = (2.0 * n / a) * J[n] - J[n + 1]
    j0_series = bessel_J(0, a)
...
print("=" * 74)
print("  FORCE V1 — la forme close de la force G* (dépôt fermé du 28/08/2026)")
print("=" * 74)
...
for a_c2 in (A_SWEEP, A_DRIFT):
    J_rec = bessel_table_recurrence(N_TRUNC + 20, a_c2)
    for n in range(0, N_TRUNC + 1):
        js = bessel_J(n, a_c2)
        if abs(js) >= 1.0e-10:
            ec2 = max(ec2, abs(J_rec[n] - js) / abs(js))
ok_global &= controle("C2 Bessel double route : série vs récurrence descendante "
                      "(a ∈ {0.1, 0.2}, n ≤ 11, |J| ≥ 1e-10)", ec2 < TOL_C, f"écart max = {ec2:.2e}")
