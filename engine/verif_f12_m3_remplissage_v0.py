# -*- coding: utf-8 -*-
"""
REMPLISSAGE F12 V0 — exécution machine de FRONTIERE_F12_M3_REMPLISSAGE_V0.md
(commit 71d92e4 — frontière gelée AVANT ce script, contrôle C0a).

MORT 3 de F12 : le remplissage. Thèse T1 (frontière §1) : la configuration
fondamentale de tout Z ∈ [1,20] émerge du triplet {lien boucle α=1 = Coulomb
−1/r (déposé b249526), capacité 2(2l+1) de l'exclusion M1 (d0f714a),
minimisation d'énergie à chaque pas de SCF} — sans règle de Madelung importée.
T2 : fermeture ⟺ I(Z)/I(Z−1) < 0.4, cible {3,11,19}. T3 : ε(4s)−ε(3d) < 0 à
Z=19,20 et > 0 à Z=10 — les deux côtés. T4 (négatif, doit tenir) : aucun β du
balayage gelé ne reproduit {2,10,18} à un corps. T5 : témoins E(H)=−0.5 ≤ 5e-4
(un corps) et E(He) ∈ [−2.88,−2.83] (Hartree, échange omis).

Contrôles bloquants C0a…C8 (frontière §2, barres gelées) — UN SEUL en échec
⟹ V4_REFUTE exit 1, sans sauvetage. Conséquences D1–D3 tranchent V+/V2/V3
(frontière §4) ; D5–D7 [OBS] consignés.

Honnêteté (frontière §5) : échange omis (Hartree, pas Hartree-Fock) — les
barres portent sur des ORDRES et des RATIOS ; convention brute avec
auto-interaction (E(H)≈−0.243 — l'ancre −0.5 est portée par le témoin à un
corps) ; Z=5 conv=False attendu (consigné d'AVANCE, config correcte) ;
fenêtre Z≤20 ; noyau ponctuel.
...
Sortie : resultat_f12_m3_remplissage_v0.json.
"""
import inspect
import json
import math
...
def scf(Z, N_e, V0=None):
    """SCF Hartree sphérique du lien boucle (α=1) → dict(E, occ, conv, stable,
    eps, V, charge). Échange omis — consigné. Occupations gelées après IT_FREEZE."""
    ...
    for it in range(MAX_ITER):
        ...
        E_tot = E_occ - E_h              # convention brute (auto-interaction incluse, §5.2)
...
print("=" * 74)
print("  REMPLISSAGE F12 V0 — MORT 3 : le tableau périodique sort-il du champ ?")
print("=" * 74)
...
FRONT = os.path.join(RACINE, "FRONTIERE_F12_M3_REMPLISSAGE_V0.md")
mtime_front = os.path.getmtime(FRONT)
c0a_ok = mtime_front < t_exec
ok_global &= controle("C0a frontière M3 antérieure à l'exécution (dépôt-d'abord)", c0a_ok,
                      f"mtime frontière "
...
# ------------------------------------------------------------------ C4 — témoin He
E_he = neutres[2]["E"]
c4_ok = (-2.88 <= E_he <= -2.83) and neutres[2]["conv"] and neutres[2]["stable"]
ok_global &= controle("C4 témoin He (Hartree, échange omis) : E ∈ [−2.88, −2.83], "
                      "conv=True, stable=True", c4_ok,
                      f"E(He) = {E_he:.6f} (exact −2.9037 ; échange omis — consigné) ; "
                      f"conv={neutres[2]['conv']} stable={neutres[2]['stable']}")
note(f"[OBS] convention brute (§5.2) : E(H) SCF = {neutres[1]['E']:.6f} "
     f"(auto-interaction incluse ; l'ancre −0.5 est portée par le témoin à un corps : "
     f"|E(1s)+0.5| = {temoin_EH:.3e})")
...
src_route = "\n".join(inspect.getsource(f) for f in
                      (remplir, scf, bloc, VH_of, integ_dr, integ_d3))
low = src_route.lower()
...
ok_global &= controle("C7 convergence SCF : conv=True pour tout Z sauf Z=5 (consigné "
                      ...
                      "conv=False — frontière §5.3)", c7_ok,
...
note("[OBS] D6 table nucléaire {2,8,20,28,50,82,126} : hors portée V0 (C-P6 par table)")
...
print(f"Résultat : resultat_f12_m3_remplissage_v0.json ({time.time() - t_exec:.1f} s)")

with open(os.path.join(RACINE, "resultat_f12_m3_remplissage_v0.json"), "w",
