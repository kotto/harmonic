...
function encodeSymptoms(text) {
  const words = text.toLowerCase().replace(/[,;.!?]/g,' ').split(/\s+/).filter(w => w.length > 1);
  const vec = {};
  for (const w of words) {
    const feats = symptomMap[w];
    if (feats) {
      for (const f of feats.split(',')) {
        if (F.includes(f)) vec[f] = (vec[f] || 0) + 1;
      }
    }
  }
  // Bigrammes
  for (let i = 0; i < words.length - 1; i++) {
    const bigram = symptomMap[words[i] + '_' + words[i+1]];
    if (bigram) {
      for (const f of bigram.split(',')) {
        if (F.includes(f)) vec[f] = (vec[f] || 0) + 0.5;
      }
    }
  }
  return vec;
