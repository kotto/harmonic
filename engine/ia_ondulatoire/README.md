...
| `physique.py` | **Module Physique Harmonique** (résultats VÉRIFIÉS, 08/08/2026) : α = π⁴e⁻⁴φ⁻⁵(√2)⁻¹(√3)⁻⁵ (99,99998 % CODATA), m_p = m_e·6π⁵ (GAGUT, 0,0018 %), masse nucléaire 0-paramètre m = Z·(m_p+m_e) + N·m_n − [SEMF + coquille HO] → **0,004 % sur 536 noyaux de la vallée** (AME2020), coquille HO (fermetures 2(n+1)(n+2)(n+3)/3, amplitude ħω/2, largeur √N), prédiction ex-ante île de stabilité (pas de fermeture forte à N=184). Frontières documentées : A<40 invalide, Q_α absolus superlourds non fiables. Routes `/api/physics/*`, certificat `validation_physique.py` (20/20) |
