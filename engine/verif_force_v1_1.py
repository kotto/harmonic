# -*- coding: utf-8 -*-
"""
FORCE V1.1 — exécution machine du dépôt DEPOT_FORCE_V1_1.md (28/08/2026, 16:31:24)

Physique VERBATIM de DEPOT_FORCE_V1.md (autorité) ; unique diff déposée : le témoin
C2′ (bug d'ordre de normalisation de la récurrence Bessel corrigé — facteur figé
AVANT mutation, amorce d'ordre 1, identité S = J₀ + 2ΣJₙ = 1 déposée ex ante).
...
Sortie : resultat_force_v1_1.json (toutes les lectures, y compris les quasi-échecs).
"""
...
DEPOT = os.path.join(RACINE, "DEPOT_FORCE_V1_1.md")
SORTIE = os.path.join(RACINE, "resultat_force_v1_1.json")
...
def bessel_table_recurrence(n_max, a):
    """Route indépendante C2′ (dépôt V1.1 §0) — Miller descendant Jₙ₋₁ = (2n/a)Jₙ − Jₙ₊₁,
    amorce d'ordre 1, facteur de normalisation figé AVANT toute mutation de la table.
    (V1 était défectueux : la boucle mutait J[0] à l'itération n=0, le facteur
    j0_series/J[0] valait ensuite 1.0 et J₁..Jₙ gardaient l'échelle brute.)"""
    J = {n_max + 1: 0.0, n_max: 1.0}
    for n in range(n_max, 0, -1):
        J[n - 1] = (2.0 * n / a) * J[n] - J[n + 1]
    facteur = bessel_J(0, a) / J[0]  # figé AVANT mutation — le correctif déposé
...
print("=" * 74)
print("  FORCE V1.1 — témoin Bessel corrigé, physique gelée (dépôt fermé du 28/08/2026)")
print("=" * 74)
...
for a_c2 in (A_SWEEP, A_DRIFT):
    J_brut = bessel_table_recurrence(N_TRUNC + 20, a_c2)
    J_rec = dict(J_brut)
    J_rec0_raw_max = max(J_rec0_raw_max, J_rec[0])  # (c) non-débordement : brut ≤ 1e23
    for n in range(0, N_TRUNC + 1):
        js = bessel_J(n, a_c2)
        if abs(js) >= 1.0e-10:
            ec2 = max(ec2, abs(J_rec[n] - js) / abs(js))
    S = J_rec[0] + 2.0 * sum(J_rec[n] for n in range(1, N_TRUNC + 1))  # (b) Σ Jₙ identité
    S_max_dev = max(S_max_dev, abs(S - 1.0))
ok_global &= controle("C2′ Bessel double route corrigée : (a) série vs récurrence (facteur figé avant "
                      "mutation, amorce d'ordre 1) ≤ 1e-12 ; (b) identité J₀+2ΣJₙ = 1 ≤ 1e-9 ; "
                      "(c) J_rec[0] brut ≤ 1e23",
                      ec2 < TOL_C and S_max_dev < TOL_U1 and J_rec0_raw_max <= 1.0e23,
                      f"(a) écart max = {ec2:.2e} ; (b) max|S−1| = {S_max_dev:.2e} ; "
                      f"(c) J_rec[0] brut max = {J_rec0_raw_max:.3e}")
