...
# ── Correcteur harmonique (french_corrector) ──────────────────────────────────
# La réponse enterprise est une jointure de faits bruts (non accentués,
# parfois malformés) — le correcteur la rend grammaticale et typographique.
# Appliqué UNIQUEMENT aux réponses françaises (heuristique lexicale) : le
# pipeline est FR, jamais appliqué à l'anglais.
_FR_MARKERS = {
    'le', 'la', 'les', 'des', 'un', 'une', 'est', 'sont', 'dans', 'pour',
    'avec', 'par', 'sur', 'chez', 'entre', 'qui', 'que', 'dont', 'diabete',
    'maladie', 'sante', 'traitement', 'vaccin', 'insuline', 'paludisme',
    'cellule', 'organe', 'etude', 'reponse', 'consequence',
}


def _looks_french(text: str) -> bool:
    words = set(w.lower().strip('.,;:!?') for w in text.split())
    return sum(1 for w in words if w in _FR_MARKERS) >= 2


def _polish_answer(answer: str) -> str:
    if not answer or len(answer) < 20 or not _looks_french(answer):
        return answer
    try:
        from french_corrector import polish_prose
        return polish_prose(answer)
    except Exception:
        return answer
...
class EnterpriseEngine:
    ...
    def ingest_text(self, department_id: str, text: str, 
        ...
        if '?' in text and ('reponse' in text.lower() or 'answer' in text.lower()):
            ...
            # Couper après « . » ou « ! » MAIS aussi sur chaque fin de ligne :
            # les lignes de CSV/tableaux (sans point final) doivent rester des
            # faits séparés — indispensable pour les livrables Excel.
    ...
    def ask(self, question: str, department_id: str,
        ...
        if confidence < min_confidence or dept.fact_count == 0:
            ...
            if top_facts:
                # 🌊 MODÈLE FINAL — décodage HWAT : les faits du département
                # « s'attendent » mutuellement (PhaseAttention) et les
                # connecteurs sortent par cohérence de phase — la réponse
                # est une PROSE, plus une jointure « | ». Fallback jointure.
                try:
                    from hwat_surface import decode_texts
                    decoded = decode_texts([f.text for f in top_facts])
                    if decoded:
                        answer = ' '.join(decoded)
                    else:
