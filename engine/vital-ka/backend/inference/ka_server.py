...
def diagnose():
    """
    Diagnostic IA pour l'application médecin (contrat apps Vital Ka).
    ...
    Returns:
        {
            "diagnoses": [{"text": ..., "score": 0-1, "secteur": ...}],
            "disclaimer": ...,
            "score_harmonique": ...,
            "niveau": ...,
            "recommandations": [...]
    ...
    """
    try:
        from harmonic_health import full_diagnostic
    except ImportError:
        return jsonify({'error': 'Module harmonic_health non disponible'}), 503

    data = request.get_json(force=True, silent=True) or {}
    symptoms = data.get('symptoms') or data.get('symptomes') or []
    if isinstance(symptoms, str):
        symptoms = [s.strip() for s in symptoms.split(',') if s.strip()]
    age = data.get('patient_age') or data.get('age')
    max_diag = max(1, min(int(data.get('max_diagnoses', 3)), 10))

    result = full_diagnostic(symptoms, None, age, None)

    # Hypothèses hiérarchisées depuis les domaines scorés
    domain_labels = {
        'infectieux': 'Syndrome infectieux (fièvre, sueurs) à explorer',
        'respiratoire': 'Atteinte respiratoire (toux, essoufflement) à explorer',
        'energetique': 'Asthénie / syndrome d\'épuisement',
        'douleur': 'Syndrome douloureux (céphalées, crampes)',
        'digestif': 'Trouble digestif (nausées, diarrhée)',
        'vestibulaire': 'Trouble vestibulaire (vertiges)',
        'nerveux': 'Anxiété / trouble du sommeil à explorer',
        'cutané': 'Atteinte cutanée (éruption, prurit)',
        'general': 'Symptômes généraux — exploration complémentaire',
    }
    domain_scores = result.get('domain_scores', {}) or {}
    n_symptoms = max(1, len(symptoms))
    diagnoses = []
    for domain, count in sorted(domain_scores.items(), key=lambda x: -x[1]):
        # Score 0-1 calibré : 1 symptôme d'un domaine ≈ 0.58, 2+ ≈ 0.8+
        score = round(min(0.97, 0.35 + 0.30 * min(1.0, count / n_symptoms) + 0.15 * min(1.0, count / 2)), 3)
        diagnoses.append({
            'text': domain_labels.get(domain, f'Hypothèse : domaine {domain}'),
            'diagnosis': domain_labels.get(domain, domain),
            'score': score,
            'secteur': domain,
        })
        if len(diagnoses) >= max_diag:
            break

    return jsonify({
        'diagnoses': diagnoses,
        'disclaimer': 'Assistance IA par résonance harmonique — pas un avis médical.',
        'score_harmonique': result.get('score'),
        'niveau': result.get('niveau'),
        'domaines': result.get('domaines'),
        'vitals_check': result.get('vitals_check'),
