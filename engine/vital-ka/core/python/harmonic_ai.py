...
class HarmonicAI:
    ...
    def ask(self, question: str) -> str:
        ...
        # 🌊 MODÈLE FINAL — décodage HWAT : si la réponse est une jointure
        # de faits (« fait1 | fait2 »), les phrases « s'attendent »
        # (PhaseAttention) et les connecteurs sortent par cohérence de
        # phase — la réponse devient une prose attentionnée. Fallback : la
        # jointure d'origine.
        if response and ' | ' in response:
            try:
                from hwat_surface import decode_texts
                parts = [p.strip() for p in response.split(' | ')]
                decoded = decode_texts(parts)
                if decoded:
        ...
        # 🗣️ Correcteur harmonique (french_corrector) : grammaire, syntaxe,
        # accents, typographie — déterministe, 0 LLM. Remplace le correctif
        # _fix_accents (sous-chaîne) de style_engine. Réponses françaises
        # uniquement (le pipeline est FR — jamais appliqué à l'anglais).
        if response and lang == 'fr' and len(response) > 20:
            try:
                from french_corrector import polish_prose
                response = polish_prose(response)
            except Exception:
                pass
        ...
        # 🗣️ Polissage final (après le wrapper) : la prose livrée est
        # grammaticalement et typographiquement correcte.
        if response and lang == 'fr' and len(response) > 20:
            try:
                from french_corrector import polish_prose
                response = polish_prose(response)
