...
# Chemin racine du projet : holographic_encoder et d'autres modules
# vivent à la racine (append : ne perturbe pas l'ordre vital-ka d'abord)
_ROOT_DIR = _os.path.dirname(_os.path.dirname(
    _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))))
if _ROOT_DIR not in _sys.path:
    _sys.path.append(_ROOT_DIR)
...
WAVE_NATIVE_INTENTS = {
    ...
    'sample': 'Échantillonner avec température/top-p/top-k',
    'tool_use': 'Résoudre et exécuter un appel d\'outil',
    'evaluate': 'Évaluer la qualité/perplexité d\'une génération',
    'math': 'Calculer une expression mathématique',
    'code': 'Générer du code classique par conversion',
}
...
class WaveIntentDetector(PatternDetector):
    ...
    WAVE_MARKERS = {
        ...
        'sample': [
            'échantillonne', 'sample', 'génère avec température',
            'température', 'temperature', 'top-k', 'top_k',
            'top-p', 'top_p', 'nucléus', 'nucleus',
            'aléatoire', 'random', 'créatif', 'stochastique',
            'sampling', 'beam', 'faisceau',
        ],
        'tool_use': [
            'utilise l\'outil', 'use tool', 'appelle la fonction',
            'call function', 'exécute', 'execute',
            'lance', 'run', 'active l\'outil',
            'fonction', 'function calling', 'tool use',
            'api', 'plugin', 'extension',
        ],
        'evaluate': [
            'évalue', 'evaluate', 'mesure la perplexité',
            'perplexity', 'perplexité', 'qualité', 'quality',
            'confiance', 'confidence', 'score de cohérence',
            'métrique', 'metric', 'benchmark',
            'entropie', 'entropy', 'évaluation',
        ],
        'math': [
            'calcule', 'calculate', 'combien font', 'combien fait',
            'racine carrée', 'racine carree', 'carré de', 'au carré',
            'puissance', 'power of', 'pourcentage', 'percent of',
            'valeur absolue', 'moitié de', 'la moitié',
            'fois', 'multiplie', 'multiply', 'divisé par', 'divided by',
            'plus', 'moins', 'additionne', 'soustrais',
            '2+', '3+', '4+', '5+', '6+', '7+', '8+', '9+',
            '×', '÷', '√',
        ],
        'code': [
            'écris une fonction', 'write a function', 'écris un programme',
            'write a program', 'implémente', 'implement', 'génère du code',
            'generate code', 'fonction python', 'python function',
            'code python', 'code javascript', 'code js',
            'algorithme', 'algorithm', 'classe', 'class',
            'squelette de', 'boilerplate', 'récursif',
        ],
    }
...
class WaveCodeGenerator:
    ...
    def _generate_wave_program(self, question: str, wave_intent: str,
        ...
        elif wave_intent == 'sample':
            return self._make_sample(question, entity)
        elif wave_intent == 'tool_use':
            return self._make_tool_use(question, entity)
        elif wave_intent == 'evaluate':
            return self._make_evaluate(question, entity)
        elif wave_intent == 'math':
            return self._make_math(question, entity)
        elif wave_intent == 'code':
            return self._make_code(question, entity)
        else:
            return self._make_query(question, entity)
    ...
    def _make_sample(self, question: str, entity: str) -> Program:
        """
        Échantillonnage ondulatoire avec température/top-p/top-k.

        Pattern :
            ψ_contexte = ENCODE question
            scores = RESONANCE_BATCH(ψ_contexte, vocabulaire)
            echantillon = DECODE(ψ_contexte, top_k=50)
            RETURN echantillon

        Le module wave_sampling.py fournit WaveSampler pour le contrôle fin
        (température, top-p, top-k, phase noise, cône de cohérence).
        """
        return Program([
            Assign("ψ_contexte", Encode(question)),
            Assign("ψ_question", Encode(entity if entity else question)),
            Assign("echantillon", Decode(Var("ψ_contexte"), top_k=50)),
            Return(Var("echantillon")),
        ])

    def _make_tool_use(self, question: str, entity: str) -> Program:
        """
        Appel d'outil par binding intention⊗outil.

        Pattern :
            ψ_intention = ENCODE question
            ψ_outil = ENCODE nom_outil
            ψ_action = BIND(ψ_intention, ψ_outil)
            resultat = DECODE(ψ_action)
            RETURN resultat

        Le module wave_tool_use.py fournit WaveToolUse pour la résolution
        complète (registre d'outils, extraction de paramètres, exécution).
        """
        # Extraire le nom d'outil de la question
        tool_name = entity if entity else "outil"
        concepts = self._extract_concepts(question)
        if concepts:
            tool_name = concepts[0]

        return Program([
            Assign("ψ_intention", Encode(question)),
            Assign("ψ_outil", Encode(tool_name)),
            Assign("ψ_action", Bind(Var("ψ_intention"), Var("ψ_outil"))),
            Assign("resultat", Decode(Var("ψ_action"))),
            Return(Var("resultat")),
        ])

    def _make_evaluate(self, question: str, entity: str) -> Program:
        """
        Évaluation de la qualité/perplexité d'une génération.

        Pattern :
            ψ_q = ENCODE question
            ψ_ref = ENCODE référence (ou question elle-même)
            coherence_score = RESONANCE(ψ_q, ψ_ref)
            RETURN coherence_score

        Le module wave_perplexity.py fournit :
          - wave_entropy(ψ), wave_perplexity(ψ)
          - coherence_perplexity(scores), confidence(scores)
          - generation_quality(sequence, vocab)
        """
        return Program([
            Assign("ψ_q", Encode(question)),
            Assign("ψ_ref", Encode(entity if entity else question)),
            Assign("coherence_score", Resonance(Var("ψ_q"), Var("ψ_ref"))),
            Assign("evaluation", Decode(Var("ψ_q"), top_k=3)),
            Return(Var("coherence_score")),
        ])

    def _make_math(self, question: str, entity: str) -> Program:
        """
        Calcul mathématique par conversion (nœuds computationnels).

        Parse l'expression française ("2 plus 3 fois 4", "racine carrée
        de 16", "15% de 200") en un arbre MathOp avec priorité des
        opérateurs, puis le convertit en code classique via wave_emit.

        Pattern :
            resultat = ADD(2, MUL(3, 4))
            RETURN resultat

        Exemple :
            "Calcule 2 plus 3 fois 4" → resultat = (2 + (3 * 4)) = 14
        """
        expr = self._parse_math_expr(question)
        if expr is None:
            return self._make_query(question, entity)
        return Program([
    ...
    def _make_code(self, question: str, entity: str) -> Program:
        """
        Génération de code par conversion (programme harmonique complet).

        Détecte l'intention/opération/entité via PatternDetector
        (code_generator.py), puis :
          - Si l'opération est dans ALGORITHM_LIBRARY → programme complet
            (définition de la fonction + appel), convertible en 3 langages.
          - Sinon → squelette computationnel (FunctionCall).

        Exemple :
            "écris une fonction factorielle" → def factorial(n): ...
        """
        code_intent = self.detector.detect(question)
        op = code_intent.operation
        entity_name = code_intent.entity or entity or 'x'

        # Résolution dans la bibliothèque d'algorithmes harmoniques
        try:
            from wave_algorithms import ALGORITHM_LIBRARY
        except ImportError:
            ALGORITHM_LIBRARY = {}

        if op in ALGORITHM_LIBRARY:
            fn = ALGORITHM_LIBRARY[op]
            return Program([
                fn,
                Assign("entree", StringLit(entity_name)),
                Assign("resultat", FunctionCall(op, [Var("entree")])),
                Return(Var("resultat")),
            ])

        # Fallback : squelette computationnel
        func_name = op or 'fonction'
        return Program([
            Assign("entree", StringLit(entity_name)),
            Assign("resultat", FunctionCall(func_name, [Var("entree")])),
            Return(Var("resultat")),
        ])

    # ═════════════════════════════════════════════════════════════════
    # PARSEUR D'EXPRESSIONS MATHÉMATIQUES FRANÇAISES
    # ═════════════════════════════════════════════════════════════════

    def _parse_math_expr(self, question: str) -> Optional[object]:
        """
        Parse une expression mathématique française en arbre MathOp.

        Priorités : SQRT/POW2 > MUL/DIV/PCT > ADD/SUB.
        Supporte : "2 plus 3", "2 fois 4", "10 moins 3", "20 divisé par 5",
        "racine carrée de 16", "carré de 5", "15% de 200", "2^3",
        parenthèses et symboles (+ - * / ( )).

        Returns:
            MathOp AST ou None si aucune expression trouvée
        """
        tokens = self._tokenize_math(question)
        if not tokens:
            return None

        self._math_tokens = tokens
        self._math_pos = 0

        try:
            expr = self._math_add_sub()
            # L'expression doit consommer tous les tokens significatifs
            if self._math_pos == len(tokens):
                return expr
        except Exception:
            pass
        return None

    def _tokenize_math(self, question: str) -> List[Tuple[str, float]]:
        """Tokenise une question en (type, valeur)."""
        q = question.lower()
        # Mots d'opérateurs → symboles
        q = re.sub(r'racine carrée de|racine carree de', '√', q)
        q = re.sub(r'valeur absolue de', 'abs', q)
        q = re.sub(r'la moitié de', '½', q)
        q = re.sub(r'divisé par|divise par|divisé par', '÷', q)
        q = re.sub(r'multiplié par|multiplie par|multiplié', '×', q)
        q = re.sub(r'carré de|carrée de', '^2 ', q)
        q = re.sub(r'au carré', '^2 ', q)
        q = re.sub(r'puissance', '^', q)
        q = re.sub(r'fois', '×', q)
        q = re.sub(r'plus', '+', q)
        q = re.sub(r'moins', '-', q)
        ...
        while i < n:
            ...
            if c.isdigit():
                ...
                tokens.append(('SQRT', 0.0)); i += 1
            elif q[i:i+3] == 'abs':
                tokens.append(('ABS', 0.0)); i += 3
            elif c == '½':
                tokens.append(('HALF', 0.0)); i += 1
            elif c == '÷':
                tokens.append(('DIV', 0.0)); i += 1
            elif c == '×':
                tokens.append(('MUL', 0.0)); i += 1
            elif c == '^':
                if q[i:i+2] == '^2' and (i + 2 >= n or not q[i+2].isdigit()):
                    tokens.append(('POW2', 0.0)); i += 2
                else:
                    tokens.append(('POW', 0.0)); i += 1
            elif c == '+':
                tokens.append(('ADD', 0.0)); i += 1
            elif c == '-':
                tokens.append(('SUB', 0.0)); i += 1
            elif c == '(':
                tokens.append(('LPAREN', 0.0)); i += 1
            elif c == ')':
                tokens.append(('RPAREN', 0.0)); i += 1
            elif c == '%':
                tokens.append(('PCT', 0.0)); i += 1
            elif c == '*' or c == 'x' and q[max(0, i-1):i] == ' ':
                # '*' seul ; 'x' comme symbole de multiplication uniquement
                if c == '*' or (i > 0 and i + 1 < n and
                                q[i-1].isdigit() and q[i+1].isdigit()):
    ...
    def _math_peek(self) -> Optional[str]:
        if self._math_pos >= len(self._math_tokens):
            return None
        return self._math_tokens[self._math_pos][0]

    def _math_next(self) -> Tuple[str, float]:
        tok = self._math_tokens[self._math_pos]
        self._math_pos += 1
        return tok

    def _math_add_sub(self):
        """expr := term (('+'|'-') term)*"""
        left = self._math_mul_div()
        while self._math_peek() in ('ADD', 'SUB'):
            op, _ = self._math_next()
            right = self._math_mul_div()
            left = MathOp(op, left, right)
        return left

    def _math_mul_div(self):
        """term := factor (('*'|'/'|'%') factor)*"""
        left = self._math_factor()
        while self._math_peek() in ('MUL', 'DIV', 'PCT', 'POW'):
            op, _ = self._math_next()
            right = self._math_factor()
            if op == 'PCT':
                # a % de b → (a/100) * b
                left = MathOp('MUL', MathOp('DIV', left, Literal(100.0)), right)
            else:
                left = MathOp(op, left, right)
        return left

    def _math_factor(self):
        """factor := NUM | SQRT(factor) | HALF(factor) | POW2(factor) | '-' factor | '(' expr ')'"""
        tok_type, tok_val = self._math_next()
        if tok_type == 'NUM':
            return Literal(tok_val)
        if tok_type == 'SUB':
            # Négation unaire : -9
            return MathOp('NEG', self._math_factor())
        if tok_type == 'SQRT':
            return MathOp('SQRT', self._math_factor())
        if tok_type == 'HALF':
            return MathOp('DIV', self._math_factor(), Literal(2.0))
        if tok_type == 'ABS':
            return MathOp('ABS', self._math_factor())
        if tok_type == 'POW2':
            return MathOp('POW', self._math_factor(), Literal(2.0))
        if tok_type == 'LPAREN':
            expr = self._math_add_sub()
            if self._math_peek() == 'RPAREN':
                self._math_next()
            return expr
        raise ValueError(f"Token math inattendu: {tok_type}")
...
def wave_to_python(program: Program) -> str:
    ...
    Délègue à wave_emit.emit_python (conversion multi-backend unifiée).
    ...
    lines = [
        ...
        ")",
        "# Modules complémentaires (sampling, tool use, perplexity)",
        "try:",
        "    from wave_sampling import WaveSampler, entropy, perplexity",
        "    from wave_tool_use import WaveToolUse, ToolDefinition",
        "    from wave_perplexity import (",
        "        wave_entropy, wave_perplexity, coherence_perplexity,",
        "        confidence, coherence_margin, generation_quality",
        "    )",
        "except ImportError:",
...
if __name__ == "__main__":
    import os as _os
    import sys as _sys
    # Chemin racine (holographic_encoder vit à la racine du projet)
    ...
    tests = [
        ...
        ("sample", "Échantillonne avec température 0.8 et top-p 0.9 sur le concept de créativité"),
        ("tool_use", "Utilise l'outil calculer pour résoudre 2+2"),
        ("evaluate", "Évalue la qualité de la réponse : le ciel est bleu"),
        ("math", "Calcule 2 plus 3 fois 4"),
        ("math", "racine carrée de 16"),
        ("math", "combien font 15% de 200"),
        ("code", "écris une fonction qui inverse une liste"),
        ("code", "implémente un algorithme de tri en python"),
    ]
