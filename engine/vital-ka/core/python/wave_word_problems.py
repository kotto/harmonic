...
class WaveWordProblemEngine:
    ...
    def solve(self, question: str) -> Optional[WordProblemResult]:
        ...
        detectors: List[Tuple[str, Callable[[str], Optional[Tuple[List[str], MathOp]]]]] = [
            ...
            ("produit de taux (per hour)", self._solve_rate_product),
            ("proportions multiples", self._solve_times_more),
    ...
    def _solve_rate_product(self, q: str) -> Optional[Tuple[List[str], MathOp]]:
        """« paid X per hour, works Y hours per week, Z weeks » → X×Y×Z.
           Produit de taux : per hour × per week × weeks."""
        # X per hour + Y hours per week + Z weeks
        m = re.search(r'(\d+(?:[.,]\d+)?)\s*(?:\$|€)?\s*per\s+(hour|day|week)'
                      r'.*?(\d+(?:[.,]\d+)?)\s*(?:hours?|days?|weeks?)\s*'
                      r'per\s+(?:week|month|day)'
                      r'.*?(\d+(?:[.,]\d+)?)\s*(?:weeks?|months?|days?)\b', q)
        if not m:
            return None
        rate, unit = _num(m.group(1)), m.group(2)
        per_period = _num(m.group(3))
        periods = _num(m.group(4))

        # Ajuster l'unité du taux : per day → per week → per month...
        mult = {'hour': 1.0, 'day': 1.0, 'week': 1.0}.get(unit, 1.0)

        expr = MathOp('MUL', MathOp('MUL', _lit(rate * mult), _lit(per_period)),
                      _lit(periods))
        step1 = rate * per_period
        total = step1 * periods
        return ([f"{_fmt(rate)} × {_fmt(per_period)} = {_fmt(step1)} par période",
                 f"{_fmt(step1)} × {_fmt(periods)} = {_fmt(total)}"], expr)

    def _solve_times_more(self, q: str) -> Optional[Tuple[List[str], MathOp]]:
        """« A has X times as many as B. If A is Y, how many does B have? »
           Chaîne de ratios : times as many, twice, half as many, a third."""
        # Ratios numériques : "4 times as many" → 4
        # Ratios lexicaux : "twice" → 2, "half as many" → 0.5, "a third" → 1/3
        ratios: List[float] = []
        for m in re.finditer(r'(\d+(?:[.,]\d+)?)\s*times as (?:much|many)',
                             q):
            ratios.append(_num(m.group(1)))
        ratios += [2.0] * len(re.findall(r'\btwice\b', q))
        ratios += [0.5] * len(re.findall(r'\bhalf as (?:much|many)\b', q))
        ratios += [1.0 / 3.0] * len(re.findall(r'\ba third\b', q))
        ratios += [0.25] * len(re.findall(r'\ba quarter\b', q))
        ratios += [3.0] * len(re.findall(r'\bthree times\b', q))
        ratios += [4.0] * len(re.findall(r'\bfour times\b', q))
        if not ratios:
            return None

        # La valeur de référence : la variable finale ("If X has # ...")
        m2 = re.search(r'(?:if|when|since|as)\s+[a-z]+\s+(?:has|is|had)\s+'
                       r'(\d+(?:[.,]\d+)?)', q)
        m1 = re.search(r'(?:has|had|is)\s+(\d+(?:[.,]\d+)?)\s*$', q)
        base = None
        if m2:
            base = _num(m2.group(1))
        elif m1:
            base = _num(m1.group(1))
        if base is None:
            return None

        # Deux modes selon la question :
        #  - "how many together/total" → multiplier depuis la base + sommer
        #  - "how many does X have"    → diviser pour descendre la chaîne
        is_total = ('together' in q or 'altogether' in q or 'in total' in q
                    or 'combined' in q or 'au total' in q or 'ensemble' in q)

        expr = _lit(base)
        if is_total:
            # Remonter : chaque ratio MULTIPLIE la valeur précédente
            values = [base]
            current = base
            for r in ratios:
                current *= r
            ...
            total = sum(values)
            steps = [f"{_fmt(base)} (base)"]
            for v in values[1:]:
                steps.append(f"× {_fmt(v / values[values.index(v) - 1])} = "
                             f"{_fmt(v)}")
            steps.append(f"total : {' + '.join(_fmt(v) for v in values)} "
                         f"= {_fmt(total)}")
            expr = _lit(values[0])
            for v in values[1:]:
                expr = MathOp('ADD', expr, _lit(v))
            return steps, expr
        else:
            # Descendre : division par chaque ratio
            result = base
            steps = [f"{_fmt(base)} (référence)"]
            for r in ratios:
                result /= r
                steps.append(f"÷ {_fmt(r)} = {_fmt(result)}")
                expr = MathOp('DIV', expr, _lit(r))
            return steps, expr
