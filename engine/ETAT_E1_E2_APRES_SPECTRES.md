...
**Mise à jour 5 (28/08/2026)** : **registre complet consolidé — les premiers secteurs comptés.** Le bilan sectoriel intégral de la campagne (vérifié sur pièces, rapports JSON à l'appui) :

*Secteurs bankés [T] :* (1) **E1a** Ĥ = ℏω₀·n̂, ≤ 4,4e-16 ; (2) **E1b** masse par courbure, κ = 0,427511045, 22/22 ; (3) **α_W = 1/30** exact ; (4) **famille T*** — 24/24 machine (E3) + états quantiques ex-ante : 20 matchs exacts (18 T*, 2 α = 1/φ), 35 quasi vs 46,75 attendus sous bruit (la numérologie ne s'accumule pas) ; (5) **campagne jauge** (27/08) — U(1) cinématique [T] + ontologique [P] (`FICHE_THEOREME_U1.md`), **SU(2) dyade 8/8 ≤ 4,37e-15** (`THEOREME_DYADE_SU2.md`), **SU(3) triangle 7/7 ≤ 8,12e-15** (`THEOREME_TRIANGLE_SU3.md`) : N modes fermés + une loi scalaire ⟹ su(N)−1 canaux, découplage U(1)×SU(N) exact — première structure du MS dérivée, sans Lagrangien ni groupe postulé ; (6) **tableau périodique 118/118** + gaz nobles + masses (V1 mono-isotopiques 8,5e-5, pic de fer ✅, vallée ⚠ SEMF empirique documenté) + T*_ion 24 températures dorées (`DOCUMENT_FONDATEUR_TABLEAU_MASSES.md`, 09/08) ; (7) **π, e dérivés** + λ = φ exact, chaîne cₙ = 1/Γ(n/φ+1) à 2,22e-16 ; (8) **convergence D_p** (seul D₂) + finding F-C1 (annulation catastrophique, corrigé Decimal 50 chiffres).

*Défaites consignées [F] :* vertex 0/15 (témoin négatif conforme au dépôt), produit K*, état doré cohérent, particules T6 ex-ante **0/26** (`tableau_periodique_report.json` : B_pas_de_match — la structure ✅, la prédiction ❌, consigné tel quel), **α grammaire statique 0/218** (5-facteurs reclassé coïncidence de compression, ancre 3,1e-12).

*Reste structurel ouvert :* le jaugage (−¼F², champs dynamiques), pourquoi N=3 couleurs / N=2 faible, la chiralité, E1c V(Φ), Yukawa/CKM, α_S. Le graphe complet : N modes ⟹ groupe dérivé, mais la *nature* des N et le *jaugage* restent à dériver.
