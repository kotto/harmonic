...
V1 — sonde de queue de Hurwitz (seul changement vs V0 : O5, n0 = N/2 = 25).
Dépôt fermé ex ante : DEPOT_OYIBO_CONSERVATION_V1.md (chaîne CH-G1..CH-G6,
objets O1..O8, barres O8, contrôles bloquants §3, échelle de verdicts §4 — gelés).

Sortie : resultat_oyibo_conservation_v1.json — toutes les lectures (I3),
...
DEPOT = os.path.join(BASE, "DEPOT_OYIBO_CONSERVATION_V1.md")
SORTIE = os.path.join(BASE, "resultat_oyibo_conservation_v1.json")
...
TAIL_N0 = N_CONV // 2   # O5-V1 : sonde de queue, indices [25, 49] (moitié de queue, non calibrée)


def nu_n(x, n=N_CONV):
    """ν*_n(x) = min sur les convergents d'indice [n0, n−1] (V1, queue — le liminf de
    Hurwitz est une propriété de queue ; le min-tête de V0 lit le transitoire c1).
    Retourne (ν*_V1, ν_V0, convergents) : le fonctionnel V0 reste consigné (I3)."""
    ...
    for i, (p, q) in enumerate(convs):
        ...
        if best_all is None or val < best_all:
            best_all = val
        if i >= TAIL_N0 and (best_tail is None or val < best_tail):
            best_tail = val
    return best_tail, best_all, convs
...
def main():
    ...
    LECTURES["controles"]["C0a"] = {
        "definition": "mtime(DEPOT_OYIBO_CONSERVATION_V1.md) < heure d'exécution",
    ...
    for (nom, x) in REGISTRE:
        val, val_v0, convs = nu_n(x)
        nu_vals[nom] = val
        nu_v0_vals[nom] = val_v0
        table_nu.append({"candidat": nom, "nu50_V1_queue": fnum(val), "nu50_V0_tete": fnum(val_v0)})
    ...
    LECTURES["famille_C"] = {
        "definition": "CH-G4 V1 : sonde de queue n0=25 (ν*), hit doré 1/√5, ancre 1/(2√2), unicité ; fonctionnel V0 (tête) consigné à titre de leçon",
        "table": table_nu,
        "C3a": {"nom": "|ν*50(1/φ) − 1/√5| (sonde de queue)", "valeur": fnum(c3a_dev), "barre": fnum(TOL_HURWITZ_HIT), "passe": bool(c3a_ok)},
        "C3b": {"nom": "|ν*50(1/√2) − 1/(2√2)| (sonde de queue)", "valeur": fnum(c3b_dev), "barre": fnum(TOL_HURWITZ_HIT), "passe": bool(c3b_ok)},
    ...
    resultat = {
        "meta": {
            "depot": "DEPOT_OYIBO_CONSERVATION_V1.md",
            "script": "verif_oyibo_conservation_v1.py",
            "sortie": "resultat_oyibo_conservation_v1.json",
    ...
    print(f"[OYIBO-CONSERVATION-V1] verdict {verdict['code']} — {verdict['nom']} (exit {verdict['exit_code']})")
