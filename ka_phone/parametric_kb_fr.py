...
class ParametricKB:
    ...
    def _load_rules(self):
        return [
            ...
            # === ARITHMÉTIQUE FRANÇAIS — patterns FR + EN (bilingue, tolère accents ou non) ===
            {"name": "addition_fr", "pattern": r'(?:combien\s+font|calculez?|calculer|quelle\s+est\s+la\s+somme|somme\s+de)\s*(\d+)\s*\+\s*(\d+)', "compute": lambda m: f"{m.group(1)} + {m.group(2)} = {int(m.group(1))+int(m.group(2))}", "domain": "arithmetic", "confidence": 0.99},
            {"name": "multiplication_fr", "pattern": r'(?:combien\s+font|calculez?|calculer|quel\s+est\s+le\s+resultat\s+de|resultat\s+de)\s*(\d+)\s*(?:fois|multipli[eé]\s+par|×|x)\s*(\d+)', "compute": lambda m: f"{m.group(1)} × {m.group(2)} = {int(m.group(1))*int(m.group(2))}", "domain": "arithmetic", "confidence": 0.99},
            {"name": "division_fr", "pattern": r'(?:combien\s+font|calculez?|calculer|quel\s+est\s+le\s+resultat\s+de|resultat\s+de)\s*(\d+)\s*(?:divis[eé]\s+par|/)\s*(\d+)', "compute": lambda m: f"{m.group(1)} ÷ {m.group(2)} = {int(m.group(1))/int(m.group(2)):.4f}" if int(m.group(2))!=0 else "Division par zéro impossible", "domain": "arithmetic", "confidence": 0.99},
            {"name": "puissance_fr", "pattern": r'(?:combien\s+font|calculez?|calculer|r[eé]sultat\s+de|quel\s+est\s+le\s+r[eé]sultat\s+de)\s*(\d+)\s*(?:puissance|\\^|ˆ)\s*(\d+)', "compute": lambda m: f"{m.group(1)}^{m.group(2)} = {int(m.group(1))**int(m.group(2))}", "domain": "arithmetic", "confidence": 0.99},
            {"name": "racine_carree_fr", "pattern": r'(?:quelle\s+est\s+la\s+)?racine\s+carr[eé]e?\s+(?:de|d\')\s*(\d+)', "compute": lambda m: f"sqrt({m.group(1)}) = {math.sqrt(int(m.group(1))):.4f}", "domain": "arithmetic", "confidence": 0.98},
            {"name": "equation_fr", "pattern": r'(?:r[eé]sous?|resoudre|r[eé]soudre|solve)\s*:?\s*(\d*)\s*x\s*([+\-])\s*(\d+)\s*=\s*(\d+)', "compute": lambda m: (lambda a,sign,b_str,c_str: f"x = ({c_str} {'-' if sign=='+' else '+'} {b_str}) / ({a} if a else 1) = {(int(c_str)+(int(b_str) if sign=='-' else -int(b_str)))//(int(a) if a else 1):.0f}")(m.group(1) or '1', m.group(2), m.group(3), m.group(4)), "domain": "algebra", "confidence": 0.96},
            {"name": "determinant_fr", "pattern": r'(?:quel\s+est\s+le\s+)?d[eé]terminant\s+(?:de|du|des?)\s*\[*\[?\s*(\d+)\s*,?\s*(\d+)\s*[,;\s]+(\d+)\s*,?\s*(\d+)\s*\]*\]?', "compute": lambda m: f"det = {int(m.group(1))}x{int(m.group(4))} - {int(m.group(2))}x{int(m.group(3))} = {int(m.group(1))*int(m.group(4))-int(m.group(2))*int(m.group(3))}", "domain": "algebra", "confidence": 0.97},
            {"name": "derivee_fr", "pattern": r'(?:quelle\s+est\s+la\s+)?d[eé]riv[eé]e?\s+(?:de|d\')\s*x\^?(\d+)', "compute": lambda m: f"d/dx(x^{m.group(1)}) = {m.group(1)}x^{int(m.group(1))-1}" if int(m.group(1))>=1 else "d/dx(constante) = 0", "domain": "calculus", "confidence": 0.97},
            {"name": "pgcd_fr", "pattern": r'(?:quel\s+est\s+le\s+)?pgcd\s+(?:de|entre)\s*(\d+)\s+(?:et|,)\s*(\d+)', "compute": lambda m: f"PGCD({m.group(1)},{m.group(2)}) = {math.gcd(int(m.group(1)),int(m.group(2)))}", "domain": "arithmetic", "confidence": 0.99},
            {"name": "secondes_heure_fr", "pattern": r'combien\s+(?:de|d\')\s*secondes?\s+dans\s+une\s+heure', "compute": lambda m: "1 heure = 3600 secondes", "domain": "arithmetic", "confidence": 0.99},
            {"name": "train_distance_fr", "pattern": r'(?:train|voiture|v[eé]hicule)\s+(?:roule|va)\s+(?:a|[aà])\s*(\d+)\s*(?:km/h|kilom[eè]tres?\s*(?:par|/)h)\s+(?:pendant|en|durant)\s*(\d+)h\s*(\d+)?', "compute": lambda m: f"Distance = {m.group(1)} × {m.group(2)}.{m.group(3) or '0'}h = {int(m.group(1))*(float(m.group(2))+float(m.group(3) or 0)/60):.1f} km", "domain": "algebra", "confidence": 0.97},
            # ═══ 9 NOUVELLES RÈGLES FR (couverture mathématique complète) ═══
            {"name": "aire_cercle_fr", "pattern": r'(?:quelle\s+est\s+l[\'e]|quel\s+est\s+le\s+)?aire\s+(?:d[\'e]un|du)\s+cercle\s+(?:de\s+rayon|r\s*=\s*|avec\s+rayon\s*)?\s*(\d+)', "compute": lambda m: f"Aire du cercle de rayon {m.group(1)} = pi x {m.group(1)}^2 = {math.pi * int(m.group(1))**2:.2f}", "domain": "geometry", "confidence": 0.97},
            {"name": "hypotenuse_fr", "pattern": r'(?:quelle\s+est\s+l[\'e]|quel\s+est\s+le\s+)?hypot[eé]nuse\s+(?:d[\'e]un|du)\s+triangle\s+rectangle\s+(?:de\s+c[oô]t[eé]s?|avec\s+c[oô]t[eé]s?)\s*(\d+)\s+(?:et|,)\s*(\d+)', "compute": lambda m: f"Hypotenuse = sqrt({m.group(1)}^2 + {m.group(2)}^2) = {math.sqrt(int(m.group(1))**2 + int(m.group(2))**2):.2f}", "domain": "geometry", "confidence": 0.97},
            {"name": "combinaisons_fr", "pattern": r'(?:combien\s+(?:de|d[\'e])\s*)?combinaisons?\s+pour\s+choisir\s+(\d+)\s+parmi\s+(\d+)', "compute": lambda m: f"C({m.group(2)},{m.group(1)}) = {m.group(2)}!/({m.group(1)}!({m.group(2)}-{m.group(1)})!) = {math.comb(int(m.group(2)), int(m.group(1)))}", "domain": "combinatorics", "confidence": 0.96},
            {"name": "factorielle_fr", "pattern": r'(?:combien\s+font|quel\s+est\s+le\s+resultat\s+de|calculez?)\s*(\d+)\s*!\s*(?:factorielle)?', "compute": lambda m: f"{m.group(1)}! = {math.factorial(int(m.group(1)))}", "domain": "arithmetic", "confidence": 0.99},
            {"name": "determinant_fr", "pattern": r'(?:quel\s+est\s+le\s+)?d[eé]terminant\s+(?:de|du|des?)\s*\[?\s*(\d+)\s*,?\s*(\d+)\s*[,;\s]+(\d+)\s*,?\s*(\d+)\s*\]?', "compute": lambda m: f"det = {int(m.group(1))}×{int(m.group(4))} - {int(m.group(2))}×{int(m.group(3))} = {int(m.group(1))*int(m.group(4))-int(m.group(2))*int(m.group(3))}", "domain": "algebra", "confidence": 0.97},
            {"name": "volume_sphere_fr", "pattern": r'(?:quelle\s+est\s+l[\'e]|quel\s+est\s+le\s+)?volume\s+(?:d[\'e]une|de\s+la|du)\s+sph[eè]re\s+(?:de\s+rayon|r\s*=\s*)?\s*(\d+)', "compute": lambda m: f"Volume de la sphère de rayon {m.group(1)} = (4/3)πr³ = {4/3*math.pi*int(m.group(1))**3:.2f}", "domain": "geometry", "confidence": 0.96},
            {"name": "volume_cylindre_fr", "pattern": r'(?:quelle\s+est\s+l[\'e]|quel\s+est\s+le\s+)?volume\s+(?:d[\'e]un|du)\s+cylindre\s+(?:de\s+rayon|r\s*=\s*)?\s*(\d+)\s+(?:et\s+(?:de\s+)?hauteur|h\s*=\s*)?\s*(\d+)', "compute": lambda m: f"Volume du cylindre = πr²h = π({m.group(1)})²({m.group(2)}) = {math.pi*int(m.group(1))**2*int(m.group(2)):.2f}", "domain": "geometry", "confidence": 0.96},
            {"name": "distance_train_fr", "pattern": r'(?:quelle\s+est\s+la\s+)?distance\s+parcourue\s+(?:par\s+un\s+(?:train|vehicule|v[eé]hicule)\s+(?:a|[aà])\s*(\d+)\s*km/h\s+pendant\s*(\d+)h(\d+)?)', "compute": lambda m: f"Distance = {m.group(1)} × ({m.group(2)}+{m.group(3) or '0'}/60) = {int(m.group(1))*float(m.group(2)):.0f} km", "domain": "algebra", "confidence": 0.97},
            {"name": "pourcentage_fr", "pattern": r'(?:quel\s+est\s+le\s+)?pourcentage\s+(?:de|d[\'e]un|du)\s*nombre\s*(\d+)\s+(?:par\s+rapport|sur)\s*(\d+)', "compute": lambda m: f"{m.group(1)}/{m.group(2)} = {int(m.group(1))/int(m.group(2))*100:.1f}%", "domain": "arithmetic", "confidence": 0.96},
            ...
            # === MATRICES (8 rules) ===
            {"name":"det_2x2","pattern":r'(?:det|determinant)\s*(?:of\s*)?\[+?\s*(\d+)\s*,?\s*(\d+)\s*[,;\s]+(\d+)\s*,?\s*(\d+)\s*\]+?',"compute":lambda m:f"det = {int(m.group(1))}x{int(m.group(4))} - {int(m.group(2))}x{int(m.group(3))} = {int(m.group(1))*int(m.group(4))-int(m.group(2))*int(m.group(3))}","domain":"algebra","confidence":0.97},
...
{"name":"gamma_beta_integrals","pattern":r"(?:gamma.*integral|beta.*integral|int.*e\\(-t\\).*t\\^|using.*gamma.*function)", "compute":lambda m:"Use Gamma/Beta for integrals: integral_0^inf t^(a-1)e^(-bt) dt = Gamma(a)/b^a. integral_0^1 t^(a-1)(1-t)^(b-1) dt = B(a,b).","domain":"calculus","confidence":0.84},
    ...
    def solve(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        # Skip overly broad patterns (clean list)
        broad_skip = {
