...
3. **Politique de réponse (123)** :
   ...
   SI (verified_mode = TRUE) ET (question_factuelle = TRUE) :
       ...
       SINON :
           GÉNÉRER réponse with citations [S1], [S2]
   SINON :
...
g) quand des sources sont fournies, générer (270) une réponse with citations obligatoires `[S1]`, `[S2]`, ... intégrées ;
...
b) un service systemd (420) gérant le startup, le restartup automatique, et la supervision du service avec configuration de limites de ressources et politiques de security ;
...
a) aide au diagnostic médical (510) avec références obligatoires aux guidelines de santé (HAS, OMS) and structured abstention quand les protocoles ne sont pas fournis ;
b) analyse de conformité financière (520) with citations obligatoires des réglementations (MIFID II, GDPR) et traçabilité complète pour audit réglementaire ;
...
d) verification de conformité industrielle (540) with citations obligatoires des normes techniques (ISO, CE) et validation automatique des spécifications contre les standards.
