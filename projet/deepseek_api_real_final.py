...
def generate_code_response(prompt: str, phi: float, alpha: float) -> str:
    ...
    return f"""# SOLUTION PYTHON - HARMONIC AI
...
def generate_math_response(prompt: str, phi: float, alpha: float) -> str:
    ...
    return f"""# SOLUTION MATHÉMATIQUE - HARMONIC AI
...
def _openai_compat_generate(prompt: str, max_tokens: int, temperature: float) -> str:
    ...
    # System prompt pour forcer l'identité Harmonic AI
    system_prompt = (
        "Tu es Harmonic AI, un assistant IA avancé développé par Harmonic AI Technologies. "
        "Tu es un modèle hybride combinant les meilleures capacités de DeepSeek et Qwen, "
        "optimisé par le moteur harmonique pour des réponses déterministes, précises et fiables. "
        "Tu as des capacités de traitement audio et vidéo via les services harmoniques. "
        "Tu réponds toujours en t'identifiant comme Harmonic AI, jamais comme DeepSeek ou Qwen. "
        "Tu es déployé sur AWS pour des performances optimales."
    )
