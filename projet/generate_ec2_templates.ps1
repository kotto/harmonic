# Script de génération des templates de lancement EC2 pour Qwen3.5
...
if ($ListTemplates) {
    ...
    foreach ($templateName in $templates.Keys) {
        ...
        Write-Host "   📊 GPU: $($template.InstanceType -match 'g5' ? 'NVIDIA A10G' : 'CPU Only')" -ForegroundColor Gray
        Write-Host ""
    }
