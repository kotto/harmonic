#!/usr/bin/env python3
"""
Script pour telecharger le modele Kimi K2.5 quantifie (demarrage automatique)
"""
...
if __name__ == "__main__":
    ...
    try:
        response = input("\nVoulez-vous commencer le telechargement maintenant? (oui/non): ").strip().lower()
    except KeyboardInterrupt:
        print("\n\n[INTERRUPT] Telechargement annule par l'utilisateur.")
        sys.exit(0)
    
    if response not in ['oui', 'o', 'yes', 'y']:
        print("\n[INFO] Telechargement annule.")
        print("Vous pouvez relancer ce script a tout moment pour commencer le telechargement.")
        input("\nAppuyez sur Entree pour quitter...")
        sys.exit(0)
