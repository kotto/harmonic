#!/usr/bin/env python3
"""
TEST DIRECT DU SERVEUR HCS API
Teste les algorithmes de compression du HCS Core Engine
"""
...
def test_dwt_compression():
    """Test de compression DWT"""
    print("\n" + "="*80)
    print("TEST 2: COMPRESSION DWT (HCS Core)")
    print("="*80)
    
    # Créer image de test
    image_file, original_size = create_test_image(640, 480, "test_hcs_dwt.jpg")
    
    # Préparer données
    frame_data = prepare_frame_data(image_file, 640, 480)
    
    print("\n[*] Compression DWT (2 niveaux) en cours...")
    start_time = time.time()
    
    try:
        # Compresser avec DWT
        thumb_bytes, subbands = HCSCoreEngine.dwt2d_encode(frame_data['Y'], 640, 480, levels=2)
        
        compression_time = time.time() - start_time
        
        # Calculer tailles
        original_size = 640 * 480 * 2  # 16-bit
        compressed_size = len(thumb_bytes)
        
        # Ajouter taille des subbands
        for sb in subbands:
            compressed_size += len(struct.pack(f'<{len(sb["LH"])}h', *sb['LH']))
            compressed_size += len(struct.pack(f'<{len(sb["HL"])}h', *sb['HL']))
            compressed_size += len(struct.pack(f'<{len(sb["HH"])}h', *sb['HH']))
        ...
        print(f"    Niveaux DWT: 2")
        print(f"    Subbands: {len(subbands)}")
...
def test_hcs_container():
    """Test du container HCS complet"""
    print("\n" + "="*80)
    print("TEST 3: CONTAINER HCS COMPLET")
    print("="*80)
    
    # Créer image de test
    image_file, original_size = create_test_image(640, 480, "test_hcs_container.jpg")
    
    # Préparer données
    frame_data = prepare_frame_data(image_file, 640, 480)
    
    print("\n[*] Construction container HCS en cours...")
    start_time = time.time()
    
    try:
        # Construire container
        hcs_container = HCSCoreEngine.build_hcs_container(
            frames=[frame_data],
            W=640,
            H=480,
            mode='HCS_SDI',
        ...
        # Calculer tailles
        original_size_total = 640 * 480 * 2 * 3  # 3 canaux 16-bit
        compressed_size = len(hcs_container)
        
        ratio = original_size_total / max(1, compressed_size)
        saving = (1 - compressed_size/original_size_total) * 100
        
        print(f"\n[+] RÉSULTATS CONTAINER HCS:")
        print(f"    Taille originale: {original_size_total:,} bytes ({original_size_total/1024/1024:.2f} MB)")
        print(f"    Taille compressée: {compressed_size:,} bytes ({compressed_size/1024/1024:.2f} MB)")
        print(f"    Ratio: {ratio:.2f}:1")
        print(f"    Économie: {saving:.2f}%")
        print(f"    Temps: {compression_time:.3f}s")
        print(f"    Vitesse: {original_size_total/1024/1024/max(0.001, compression_time):.2f} MB/s")
        
        # Sauvegarder container
        output_file = "test_hcs_container.hcs2"
        with open(output_file, 'wb') as f:
            f.write(hcs_container)
        print(f"    Container sauvegardé: {output_file}")
        
        return {
            'method': 'HCS Container',
            'original_size': original_size_total,
            'compressed_size': compressed_size,
            'ratio': ratio,
            'saving': saving,
            'time': compression_time,
            'speed_mbps': original_size_total/1024/1024/max(0.001, compression_time),
            'mode': 'HCS_SDI',
            'dwt_levels': 2
        }
...
def test_different_resolutions():
    ...
    print("\n" + "="*80)
    print("TEST 4: COMPRESSION HCS AVEC DIFFÉRENTES RÉSOLUTIONS")
    print("="*80)
...
if __name__ == "__main__":
    try:
        ...
        # Test DWT
        result_dwt = test_dwt_compression()
        
        # Test Container HCS
        result_container = test_hcs_container()
        ...
        print(f"{'Méthode':<20} {'Ratio':<10} {'Économie':<10} {'Vitesse':<15}")
        print("-" * 60)
        
        if result_delta:
            print(f"{'Delta-H':<20} {result_delta['ratio']:>8.2f}:1 {result_delta['saving']:>8.2f}% {result_delta['speed_mbps']:>13.2f} MB/s")
        
        if result_dwt:
            print(f"{'DWT':<20} {result_dwt['ratio']:>8.2f}:1 {result_dwt['saving']:>8.2f}% {result_dwt['speed_mbps']:>13.2f} MB/s")
        
        if result_container:
            print(f"{'HCS Container':<20} {result_container['ratio']:>8.2f}:1 {result_container['saving']:>8.2f}% {result_container['speed_mbps']:>13.2f} MB/s")
        ...
        summary = {
            ...
            'dwt': result_dwt,
            'hcs_container': result_container,
