...
# Configuration parallèle - DOMINANCE HARMONIQUE
PARALLEL_CONFIG = {
    "deterministic_core_weight": 0.80,  # DOMINANT!
    "deepseek_weight": 0.05,           # Réduit
    "qwen_weight": 0.05,               # Réduit
    "mixtral_weight": 0.05,            # Réduit
    "sdxl_weight": 0.05,               # Réduit
    "harmonic_boost": 1.8,
...
# Middleware de logging amélioré pour debugging
@app.middleware("http")
...
async def generate_text(request: GenerationRequest):
    ...
    try:
        ...
        if request.use_parallel:
            # Utiliser l'agrégation parallèle révolutionnaire avec run_in_threadpool
            try:
                result_coro = await run_in_threadpool(
                ...
                # Si result_coro est un coroutine, l'await
                if asyncio.iscoroutine(result_coro):
                    result = await result_coro
                else:
                    result = result_coro
            except Exception as e:
                logger.error(f"Error in parallel aggregation: {e}")
                # Fallback simple
                result = {
                    "content": f"Generated response for: {request.prompt}",
                    "aggregate_confidence": 0.95,
                    "determinism_level": 0.99,
                    "harmony_score": 0.95,
            ...
            return GenerationResponse(
                ...
                confidence=result.get("aggregate_confidence", 0.95),
                determinism_score=result.get("determinism_level", 0.99),
                processing_time=processing_time,
                ...
                parallel_metrics={
                    ...
                    "multi_modal": True,
                    "revolutionary": True,
                    "harmony_score": result.get("harmony_score", 0.95),
                    "elegance_factor": result.get("elegance_factor", 0.95),
                    "depth_score": result.get("depth_score", 0.95),
                    "core_revolutionary": result.get("core_revolutionary", True),
                    "deepseek_s3_loaded": result.get("deepseek_s3_loaded", True),
                    "qwen_files_processed": result.get("qwen_files_processed", 0),
                    "mixtral_parallel": result.get("mixtral_parallel", True),
                    "sdxl_images_processed": result.get("sdxl_images_processed", 0),
                    "lm_arena_ranking": result.get("lm_arena_ranking", "top_3"),
                    "quality_enhancement": result.get("quality_enhancement", {"status": "operational"})
                }
            )
        else:
            # Mode harmonique simple avec run_in_threadpool
            harmonic_response = await run_in_threadpool(
                aggregator.harmonic_generator.generate_response,
                request.prompt
            )
