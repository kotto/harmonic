...
class HarmonicS3Uploader:
    ...
    def _create_global_manifest(self, results: Dict[str, Any]):
        ...
        manifest = {
            ...
            'infrastructure': {
                'foundation': {
                    'files': results.get('harmonic_ai/foundation', {}).get('files', []),
                    'status': 'uploaded'
                },
                'core': {
                    'files': results.get('harmonic_ai/core', {}).get('files', []),
                    'status': 'uploaded'
                },
                'domains': {
                    'visual': {
                        'files': results.get('harmonic_ai/domains/visual', {}).get('files', []),
                        'status': 'uploaded'
                    },
                    'mathematics': {
                        'files': results.get('harmonic_ai/domains/mathematics', {}).get('files', []),
                        'status': 'uploaded'
                    },
                    'code': {
                        'files': results.get('harmonic_ai/domains/code', {}).get('files', []),
                        'status': 'uploaded'
                    }
                }
            }
...
def main():
    ...
    # Configuration directe des credentials
    os.environ["AWS_ACCESS_KEY_ID"] = "AKIAIOSFODNN7EXAMPLE"
    os.environ["AWS_SECRET_ACCESS_KEY"] = "wJalrWU6H1Fvkgu/EXAMPLEKEY"
    os.environ["HARMONIC_BUCKET"] = "harmonic-ai-knowledge-base"
