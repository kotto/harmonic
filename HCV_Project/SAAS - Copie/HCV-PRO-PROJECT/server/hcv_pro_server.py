...
# Import Deepseek MOE integration
try:
    from deepseek_moe_aws_integration import add_deepseek_endpoints
    DEEPSEEK_AVAILABLE = True
    print("✅ Deepseek MOE integration loaded")
except ImportError as e:
    DEEPSEEK_AVAILABLE = False
    print(f"⚠️ Deepseek MOE not available: {e}")
...
def api_media(media_id):
    ...
    # Full file response with Accept-Ranges header — streaming via send_file
    response = make_response(send_file(media_path, mimetype=mime_type, conditional=True))
...
def api_status():
    ...
    # Vérifier Deepseek MOE
    status['deepseek_moe'] = {
        'available': DEEPSEEK_AVAILABLE,
        'harmonic_layer': True if DEEPSEEK_AVAILABLE else False
    }
    
    return jsonify(status)


# Ajouter les endpoints Deepseek si disponible
if DEEPSEEK_AVAILABLE:
    add_deepseek_endpoints(app)
    print('✅ Deepseek MOE endpoints added')
...
if __name__ == '__main__':
    ...
    if DEEPSEEK_AVAILABLE:
        print(f'  🌊 Deepseek MOE:')
        print(f'    POST /api/deepseek/init          Initialiser compresseur')
        print(f'    POST /api/deepseek/compress       Compresser modèle')
        print(f'    GET  /api/deepseek/models         Lister modèles')
        print(f'    POST /api/deepseek/models/<name>/generate  Générer texte')
        print(f'    GET  /api/deepseek/health          État service')
    print('=' * 60)
    print()
