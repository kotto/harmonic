...
def api_decode_hcvb():
    ...
    try:
        # ✅ Utiliser le decodeur optimise x6 GRATUIT
        from hcv_video_boost_optimized import HCVVideoBoostOptimized
        if 'file' not in request.files:
        ...
        codec = HCVVideoBoostOptimized(num_threads=4)
        decode_stats = codec.decode_pipelined(hcvb_path, restored_path)
        ...
        with open(hcvb_path, 'rb') as f:
            raw = f.read(codec.HEADER_PADDED)
        _, ver, qi, orig_w, orig_h, new_w, new_h, fps100, comp_size, _ = \
            struct.unpack('<4sBBHHHHII2s', raw[:struct.calcsize('<4sBBHHHHII2s')])
        ...
        result = {
            ...
            'quality': quality_names.get(qi, 'unknown'),
            'fps': fps100 / 100.0,
