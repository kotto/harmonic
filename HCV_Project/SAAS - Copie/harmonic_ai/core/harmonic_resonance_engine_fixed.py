...
class HarmonicResonanceEngine:
    ...
    def apply_resonance(self, signal: np.ndarray) -> Tuple[np.ndarray, ResonanceMetrics]:
        ...
        # Étape 1: Application matrice de résonance (si signal taille 64)
        if len(signal) == 64:
            resonated = np.dot(self.foundation.resonance_matrix, signal)
        else:
            # Pour les autres tailles, application pondérée
            resonated = signal.copy()
...
"""
    ...
    def _validate_engine(self) -> None:
        ...
        # Test fonctionnel - signal 64 éléments exactement pour matrice 64x64
        test_signal = np.array([1.0, 0.0, -1.0, 0.5, 0.0, 1.0, -0.5, 0.0, 1.0, 0.0, -1.0, 0.5, 0.0, 1.0, -0.5, 0.0, 1.0, 0.0, -1.0, 0.5, 0.0, 1.0, -0.5, 0.0, 1.0, 0.0, -1.0, 0.5, 0.0, 1.0, -0.5, 0.0, 1.0, 0.0, -1.0, 0.5, 0.0, 1.0, -0.5, 0.0, 1.0, 0.0, -1.0, 0.5, 0.0, 1.0, -0.5, 0.0, 1.0, 0.0, -1.0, 0.5])
