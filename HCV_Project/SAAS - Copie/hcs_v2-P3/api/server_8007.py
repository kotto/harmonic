#!/usr/bin/env python3
"""
HCS V2 API Server - Version port 8007 pour éviter les conflits
"""
...
async def compress_video(
    ...
    try:
        ...
        for frame in sample_frames:
            ...
            compressed_frame_data.append(compressed_data)
        ...
        # Créer une vidéo compressée simulée (concaténation des frames compressées)
        simulated_compressed_video = b''.join(compressed_frame_data)
        
        # Génération ID résultat
        result_id = f"vid_{result_counter}"
        result_counter += 1
        
        # Stockage résultat avec vraies données compressées
        compression_results[result_id] = {
            "original_filename": file.filename,
            "original_size": len(video_data),
            "compressed_size": len(simulated_compressed_video),
            ...
            "compressed_data": base64.b64encode(simulated_compressed_video).decode('utf-8'),
            "timestamp": time.time()
        }
        ...
        response = {
            ...
            "space_saved_percent": (1 - len(simulated_compressed_video) / len(video_data)) * 100 if len(video_data) > 0 else 0,
...
async def download_compressed(result_id: str):
    ...
    try:
        ...
        if 'compressed_data' in result:
            ...
            if 'video' in result.get('content_type', ''):
        ...
        elif 'estimated_compressed_size' in result and 'compressed_data' not in result:
...
if __name__ == "__main__":
    print("🚀 Démarrage HCS V2 API Server (Port 8007)")
    print("=" * 60)
    print("📊 Configuration:")
    ...
    print()
    print("🌐 Endpoints:")
    print("   http://localhost:8007/docs")
    print("   http://localhost:8007/redoc")
    print("   http://localhost:8007/api/v2/compress/image")
    print("   http://localhost:8007/api/v2/compress/video")
    print("   http://localhost:8007/api/v2/stats")
    print("   http://localhost:8007/api/v2/health")
    print("   http://localhost:8007/hcs_dashboard_v2")
    print()
    
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=8007,  # Port 8007 pour éviter les conflits
        reload=False,
        log_level="info"
    )
