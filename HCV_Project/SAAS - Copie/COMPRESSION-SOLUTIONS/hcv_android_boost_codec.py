...
class HCVAndroidBoostCodec:
    ...
    def decode(self, container: bytes) -> Tuple[np.ndarray, Dict]:
        ...
        Pipeline:
          ...
          2. Decompress zstd
          3. Decode H264
          4. Upscale Lanczos → résolution originale
          5. Adaptive sharpen
        """
        t0 = time.perf_counter()
        ...
        # 4. Upscale Lanczos → résolution originale
        if frame_bgr.shape[0] != orig_h or frame_bgr.shape[1] != orig_w:
            upscaled = lanczos_upscale(frame_bgr, orig_h, orig_w)
        else:
            upscaled = frame_bgr
        
        # 5. Adaptive sharpen (compense le cycle down/up)
        sharpen_strength = {
            'ultra': 0.3,
            'high': 0.4,
            'balanced': 0.5,
