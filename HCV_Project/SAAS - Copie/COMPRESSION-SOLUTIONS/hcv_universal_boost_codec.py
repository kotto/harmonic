...
class HCVUniversalBoost:
    ...
    def decode_image(self, container: bytes) -> Tuple[np.ndarray, Dict]:
        ...
        elapsed = time.perf_counter() - t0
        return frame, {'quality': quality, 'mode': mode,
    ...
    def decode_video_frame(self, container: bytes, frame_idx: int) -> Tuple[np.ndarray, Dict]:
        ...
        if mode == MODE_ZSTD:
            ...
            elapsed = time.perf_counter() - t0
            return frame, {'quality': quality, 'decode_ms': round(elapsed*1000, 1)}
