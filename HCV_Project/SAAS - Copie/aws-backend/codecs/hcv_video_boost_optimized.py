...
class HCVVideoBoostOptimized:
    ...
    def decode_pipelined(self, hcvb_path, output_path, target_scale=1.0):
        ...
        # ✅ COMPATIBILITÉ RETOUR ANCIENS FICHIERS HCV:
        # Si header trop petit → fichier ancien format → on remplace par valeurs par défaut
        if len(header) < 16:
            # Fichier ancien format (avant v1.2)
            magic = b'HCVB'
            ver = 1
            qi = 1
            ...
            fps100 = 3000
            size = os.path.getsize(hcvb_path)
            pad = b'\x00\x00'
        else:
