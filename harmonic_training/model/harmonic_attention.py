...
class SignatureProjection(nn.Module):
    ...
    NOUVEAU : Contexte temporel via convolution 1D sur les signatures.
    Les signatures d'un token dependent maintenant du contexte local
    (3 tokens avant/apres) pour une coherence temporelle.
    ...
    def __init__(self, hidden_size, dropout=0.1):
        ...
        # NOUVEAU : Convolution 1D pour le contexte temporel des signatures
        # kernel_size=7 => contexte de 3 tokens avant + 3 apres
        self.temporal_conv = nn.Conv1d(
            in_channels=7,
            out_channels=7,
            kernel_size=7,
            padding=3,  # padding pour garder la meme longueur
            groups=7,   # depthwise : chaque dimension convoluee separement
            bias=False
        )
        nn.init.dirac_(self.temporal_conv.weight)  # init = identite
        
        # Gate adaptative : combien de contexte temporel incorporer
        self.temporal_gate = nn.Parameter(torch.tensor(0.3))
    ...
    def forward(self, hidden_states):
        ...
        # NOUVEAU : Contexte temporel via convolution 1D
        # signatures: [B, S, 7] -> permute -> [B, 7, S] pour Conv1D
        sig_permuted = signatures.permute(0, 2, 1)  # [B, 7, S]
        sig_context = self.temporal_conv(sig_permuted)  # [B, 7, S]
        sig_context = sig_context.permute(0, 2, 1)  # [B, S, 7]
        
        # Fusion avec gate adaptative
        signatures = signatures * (1 - torch.sigmoid(self.temporal_gate)) + \
                     sig_context * torch.sigmoid(self.temporal_gate)
