...
function diagnose() {
  ...
  setTimeout(() => {
    ...
    for (const r of scores.slice(0, 5)) {
      ...
      h += '<div style="margin-top:8px;padding:12px;background:var(--surface-container-lowest);border-radius:8px;border-left:3px solid var(--primary-container);font-size:14px">▶ ' + r.c + '</div>';
      h += '</div>';
    }
...
KA_SECURE.showLockScreen(function () {
  ...
  // Demo buttons
  [['COVID-19', 'fievre, toux seche, fatigue, anosmie'], ['Infarctus', 'douleur thoracique, essoufflement, sueurs froides, nausees'], ['Paludisme', 'fievre cyclique, frissons, sueurs, maux de tete'], ['Dengue', 'fievre elevee, douleurs articulaires, eruption cutanee']].forEach(([n, t]) => {
    const b = document.createElement('button');
